...
public class AnsweredFragment extends BaseFragment {
    ...
    protected void initview() {
        view = LayoutInflater.from(MyApp.activity).inflate(R.layout.fragment_answered, null);
        mine_recycler =(RecyclerView)view.findViewById(R.id.mine_recycler);

        mine_recycler.setFocusable(false);


        mine_recycler.setLayoutManager(new LinearLayoutManager(MyApp.activity,LinearLayoutManager.VERTICAL,false));
        MineRecyclerAdapter mineRecyclerAdapter = new MineRecyclerAdapter(MyApp.activity,list);
        mine_recycler.setAdapter(mineRecyclerAdapter);
