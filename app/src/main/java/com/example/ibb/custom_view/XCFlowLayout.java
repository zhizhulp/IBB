...
//    添加方法，随便添加
    public void addViews(int key,View view,LayoutParams params){
    ...
    public void romeViews(int key){

        for(int i=0;i<mAllChildViews.size();i++){

            for(int j=0;j<mAllChildViews.get(i).size();j++){

                if(mAllChildViews.get(i).get(j).getTag().equals(key)){

                    this.removeView(mAllChildViews.get(i).get(j));//删除当前View，根据当前的列表id
